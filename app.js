...
const appData = {
  "examBoards": [
    ...
    },
    {
      ...
      "type": "University",
      "description": "University Grants Commission recognized universities",
      "totalPapers": 1200,
      "subjects": ["Commerce", "Management", "Computer Science", "Economics", "English", "Mathematics"],
      "classes": ["B.Com", "BBA", "BCA", "BA", "B.Sc", "M.Com", "MBA", "MCA", "MA", "M.Sc"]
    },
    {
      "id": 8,
      "name": "Graduation",
      ...
      "totalPapers": 800,
      "subjects": ["Commerce", "Science", "Arts", "Engineering", "Law"],
      "classes": ["B.Com", "B.Sc", "BA", "B.Tech", "LLB"]
    },
    {
      "id": 9,
      "name": "Postgraduate",
      ...
      "totalPapers": 600,
      "subjects": ["Commerce", "Science", "Arts", "Engineering", "Law"],
      "classes": ["M.Com", "M.Sc", "MA", "M.Tech", "LLM"]
    },
  ],
  "examPapers": [
    {
      ...
      {
        "id": 1001,
        "title": "UGC NET Commerce June 2025",
        "examBoard": "UGC Universities",
        "year": 2025,
        "subject": "Commerce",
        "class": "M.Com",
        "difficulty": "Hard",
      ...
      {
        "id": 1002,
        "title": "B.Sc Computer Science Final Exam 2024",
        "examBoard": "Graduation",
        "year": 2024,
      ...
      {
        "id": 1003,
        "title": "MA English Literature Semester 2 2025",
        "examBoard": "Postgraduate",
        "year": 2025,
        "subject": "English",
        "class": "MA",
        "difficulty": "Medium",
  ...
  "subscriptionPlans": [
    ...
    },
    {
      ...
      "features": [
        ...
        "Ad-free experience",
        "Priority support",
        "Access to AI Chat Agent (Textcraft personal support)",
        "All features of Student Premium"
      ],
      "popular": true // Make Elite plan visible and highlighted
    },
    {
      "name": "Institutional",
      "price": 14999,
      ...
      "features": [
        ...
        "Advanced analytics",
        "Priority support",
        "All features of Student Elite"
      ],
      "popular": false
    }
  ]
// Backend endpoint customization for Elite plan (stub)
// Example: POST /api/subscribe-elite
// Implement backend logic to handle Student Elite plan subscription and chat agent access
// fetch('/api/subscribe-elite', { method: 'POST', body: JSON.stringify({ userId, plan: 'Student Elite' }) })
};
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Initialize Chat Agent for Student Elite users
  initializeChatAgent();
});
// Chat Agent UI for Student Elite users
function initializeChatAgent() {
  // Add chat button to the page (bottom right corner)
  const chatBtn = document.createElement('button');
  chatBtn.id = 'chatAgentBtn';
  chatBtn.className = 'chat-agent-btn';
  chatBtn.innerHTML = '💬 Ask Tutor';
  document.body.appendChild(chatBtn);

  // Create chat modal
  const chatModal = document.createElement('div');
  chatModal.id = 'chatAgentModal';
  chatModal.className = 'chat-agent-modal hidden';
  chatModal.innerHTML = `
    <div class="chat-agent-header">
      <span>Textcraft Personal Tutor</span>
      <button id="chatAgentClose">×</button>
    </div>
    <div class="chat-agent-messages" id="chatAgentMessages"></div>
    <div class="chat-agent-input">
      <input type="text" id="chatAgentInput" placeholder="Type your question..." />
      <button id="chatAgentSend">Send</button>
    </div>
    <div class="chat-agent-note">*Available for Student Elite users only</div>
  `;
  ...
  chatBtn.addEventListener('click', function() {
    if (!isStudentElite()) {
      alert('Chat Agent is available for Student Elite users only.');
      return;
    }
    chatModal.classList.remove('hidden');
    document.getElementById('chatAgentInput').focus();
  });

  // Close chat modal
  document.getElementById('chatAgentClose').addEventListener('click', function() {
    chatModal.classList.add('hidden');
  });

  // Send message
  document.getElementById('chatAgentSend').addEventListener('click', sendChatAgentMessage);
  document.getElementById('chatAgentInput').addEventListener('keydown', function(e) {
    if (e.key === 'Enter') sendChatAgentMessage();
  });
}

function isStudentElite() {
  // Simulate plan check (replace with real user plan check)
  // For demo, only allow Elite users
  // Replace with actual user plan check in production
  const userPlan = window.currentUserPlan || 'Student Elite'; // Simulate global user plan
  return userPlan === 'Student Elite';
}

function sendChatAgentMessage() {
  const input = document.getElementById('chatAgentInput');
  const messages = document.getElementById('chatAgentMessages');
  const text = input.value.trim();
  if (!text) return;
  // Show user message
  const userMsg = document.createElement('div');
  userMsg.className = 'chat-agent-message user';
  userMsg.textContent = text;
  messages.appendChild(userMsg);
  input.value = '';
  // Only allow Elite users to use chat agent
  if (!isStudentElite()) {
    const aiMsg = document.createElement('div');
    aiMsg.className = 'chat-agent-message ai';
    aiMsg.textContent = 'Chat Agent is available for Student Elite users only.';
  ...
  fetch('/api/chat-agent', {
    ...
    .then(data => {
      const aiMsg = document.createElement('div');
      aiMsg.className = 'chat-agent-message ai';
      aiMsg.textContent = data.answer || 'Sorry, no answer available.';
      messages.appendChild(aiMsg);
      messages.scrollTop = messages.scrollHeight;
    })
    .catch(() => {
      const aiMsg = document.createElement('div');
      aiMsg.className = 'chat-agent-message ai';
      aiMsg.textContent = 'Sorry, there was a problem connecting to the tutor.';
      messages.appendChild(aiMsg);
      messages.scrollTop = messages.scrollHeight;
    });
}

// getChatAgentResponse removed: now handled by backend
// Chat Agent styles
const chatAgentStyles = document.createElement('style');
chatAgentStyles.textContent = `
.chat-agent-btn {
  position: fixed;
  bottom: 32px;
  right: 32px;
...
.chat-agent-modal.hidden { display: none; }
.chat-agent-header {
  background: #1FB8CD;
  color: #fff;
  padding: 12px 16px;
...
`;
document.head.appendChild(chatAgentStyles);
...
window.selectPlan = function(planName) {
  ...
  if (planName === 'Student Premium') {
    // Set payment amount to 299/year
    alert('Redirecting to payment for Student Premium: ₹299/year');
    // TODO: Integrate payment gateway here
  } else if (planName === 'Student Elite') {
    alert('Redirecting to payment for Student Elite: ₹499/year');
    // TODO: Integrate payment gateway here
  } else if (planName === 'Institutional') {
    alert('Redirecting to payment for Institutional: ₹14999/month');
    // TODO: Integrate payment gateway here
  } else {
